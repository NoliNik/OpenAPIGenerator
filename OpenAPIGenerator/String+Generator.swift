...
\(genNonClassAccessLevel) enum FileValue {
    ...
    func moyaFormData(name: String) -> MultipartFormData {
        ...
        case .url(let value, let fileName, let mimeType):
            return MultipartFormData(provider: .file(value), name: name, fileName: fileName, mimeType: mimeType)
        }
    }
}
    ...
    func mappedError() -> Result<Success, ServerError>  {
        return mapError { (error: Error) -> ServerError in
            if let error = error as? MoyaError, case .underlying(let underlying, _) = error {
                ...
                } else if let afError = underlying as? AFError, let afUnderlying = afError.underlyingError {
                    return ServerError.connection(afUnderlying)
                } else {
